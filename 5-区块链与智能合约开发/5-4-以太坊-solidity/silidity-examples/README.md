...
| 目录 | 内容 |
|------|------|
| `smartContracts/` | 基础语法（16 个示例） |
| `eth-basic/` | 进阶语法（继承、事件、modifier 等） |
...
```bash
FOUNDRY_PROFILE=default forge build    # 基础合约
FOUNDRY_PROFILE=advanced forge build   # 进阶合约
```
...
| 示例文件 | 测试文件 |
|----------|----------|
| `smartContracts/01.integer.sol` | `test/basic/Basic01Integer.t.sol` |
| `smartContracts/16-mapping.sol` | `test/basic/Basic16Mapping.t.sol` |
| `eth-basic/22.修饰器modifier.sol` | `test/advanced/Adv22Modifier.t.sol` |
| `eth-basic/33.HTCoinERC20.sol` | `test/advanced/Adv33HTCoin.t.sol` |
| … | 其余文件同理 |
...
1. 重新打开 `eth-basic/00.template.sol`
...
- `eth-basic/17.不要使用var.sol`、`20.全局变量.sol` 等仅作语法演示，无对应测试
