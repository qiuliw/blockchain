...
// RSA签名
func SignatureRSA(plainText []byte, priv *rsa.PrivateKey) ([]byte, error) {
	// 打开私钥文件
	...
	// 使用私钥对哈希值进行签名(说明你使用了哪个哈希算法，以便验证时使用相同的算法)
...
// RSA验签
func VerifySignatureRSA(plainText, signature []byte, pub *rsa.PublicKey) error {
	// 计算消息的哈希值
	hash := sha256.Sum256(plainText)
	// 使用公钥验证签名(说明接受的算法，不由签名者选择，防止签名者使用弱哈希算法进行签名，降低安全性)
...
func TestRsaSign() {
	...
	signature, err := SignatureRSA(msg, nil)
	if err != nil {
		panic(err)
	}

	err = VerifySignatureRSA(msg, signature, pub)
	if err != nil {
		fmt.Println("验证失败")
	} else {
		fmt.Println("验证成功")
